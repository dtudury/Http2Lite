...
function httpToFrame (streamId, type, payload, endStreamOrIsAck, endsHeaders, padLength, priority, streamDependency, isExclusive) {
  const flagsByte = _flagsToByte(endStreamOrIsAck, endsHeaders, padLength, priority)
  if (priority) {
  ...
  return ui8aHelpers.concat([_createFrameHeader(streamId, type, payload.length, flagsByte), payload], payload.length + 9)
}
