...
function decodeInteger (buffer, bitOffset, byteOffset = 0) {
  ...
  const firstByte = ui8aHelpers.readUInt8(buffer, byteOffset++)
  ...
  if (value === mask) {
    ...
    do {
      ...
      nextOctet = ui8aHelpers.readUInt8(buffer, byteOffset++)
...
function encodeInteger (byte, bitOffset, value) {
  ...
  if (value < mask) {
    return ui8aHelpers.fromCharArray([byte | value])
  }
  ...
  bytes.push(value)
  return ui8aHelpers.fromCharArray(bytes)
}
