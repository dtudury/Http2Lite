...
function _encodeLiteralHeader (encodedIndex, name, value) {
  if (value) {
    return ui8aHelpers.concat([encodedIndex, name, value])
  } else {
    return ui8aHelpers.concat([encodedIndex, name])
  }
}
...
function encodeHeader (kind, index, name, value) {
  ...
  if (kind === INDEXED) {
    ...
    encodedHeader = _encodeLiteralHeader(encodeInteger(INCREMENTAL, 2, index), name, value)
  } else if (kind === SIZE_UPDATE) {
    encodedHeader = encodeInteger(SIZE_UPDATE, 3, index)
  } else if (kind === NEVER_INDEXED) {
    encodedHeader = _encodeLiteralHeader(encodeInteger(NEVER_INDEXED, 4, index), name, value)
  } else if (kind === NOT_INDEXED) {
    encodedHeader = _encodeLiteralHeader(encodeInteger(NOT_INDEXED, 4, index), name, value)
  } else {
